...
class SAMRecordsReader(object):
    ...
    def __init__(self, filename, applicationParams=None,
        ...
        self.scoringScheme = None  # Set in readAlignments
    ...
    def _lineToHSP(self, line):
        ...
        if line[0] not in '$@':
            ...
            if refSeqName != '*':
                ...
                if self.scoringScheme is None:
                    # For first HSP in file, need to assign scoring scheme
                    try:
                        # AS is most accurate score because it is:
                        # Alignment score generated by aligner.
                        score = optionalDict['AS']
                        self.scoringScheme = 'AS'
                    except KeyError:
                        if 'M' in cigar:
                            try:
                                # MD is needed if 'M' is in the cigar string,
                                # 'M' can mean match or mismatch.
                                MD = optionalDict['MD']
                            except KeyError:
                                raise ValueError("No optional tag MD. "
                                                 "Run function findMD before "
                                                 "proceeding")
                                # findMD is in dark/sam/hacks.py and uses
                                # samtools
                            else:
                                score = self._convertCigarMD(cig, MD=MD)
                                self.scoringScheme = 'bit'
                        else:
                            score = self._convertCigarMD(cig)
                            self.scoringScheme = 'bit'
                elif self.scoringScheme == 'AS':
                    score = optionalDict['AS']
                    if KeyError:
                        raise ValueError("Mixed scoring scheme for "
                                         "query %s. Assigned scoring "
                                         "scheme is %s."
                                         % (query, self.scoringScheme))
                elif self.scoringScheme == 'bit':
                    if 'M' in cigar:
                        try:
                            MD = optionalDict['MD']
                        except KeyError:
