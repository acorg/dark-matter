...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description=("Analyse isolate sequencing data."))

    parser.add_argument(
        "--alignmentFile",
        help=(
            "The name of the alignment file. Either a BAM file or a JSON "
    ...
    parser.add_argument(
        "--clinicalSequence",
        help=(
            "The FASTA file of the sequence of the clinical isolate that "
    ...
    parser.add_argument("--sampleName", help="The name of the sample.")

    parser.add_argument(
        "--minorVariantFrequencyCutoff",
        default=0.05,
        type=float,
    ...
    parser.add_argument(
        "--minorVariantCoverageCutoff",
        default=5,
        type=int,
        help=(
            "The minimum number of reads required to evaluate a sequence "
            "for minority variants."
        ),
    )

    parser.add_argument(
        "--alternativeReferenceGB",
        default=False,
        help=(
            "Either a genbank file containing the features to be used, or "
    ...
    if args.alternativeReferenceGB:
        ...
        if args.notSARS2:
            print(
                "Please specify an alternative reference via "
    ...
    if args.alignmentFile.endswith(".json"):
        ...
        mvInfo = MinorVariantInfo(
            bamFile=args.alignmentFile, minBaseQuality=35, minMappingQuality=30
        )
        referenceLengths = SAMFilter(
            args.alignmentFile, filterRead=Reads().filter().filterRead
        ).referenceLengths()
    ...
    if mvInfo.length < consensusLen:
        print(
            f"Length of the alignment ({mvInfo.length}) is shorter than "
        ...
        for position in range(consensusLen):
            ...
            offsetInfo = offsetInfoMultipleGenomes(
                [clinicalAlignment, consensusAlignment],
                position,
                featureName=feature,
            ...
            if isMinorVariantPosition(
                mvInfo.countsPerBase[position],
                ...
                print(
                    "\t\tMinority variant at position: %d (%s), "
                    "coverage: %d reads"
                    % (
                        position + 1,
                        offsetInfo["featureName"],
                ...
                for base in mvInfo.countsPerBase[position]:
                    if (
                        mvInfo.countsPerBase[position][base]
                        > args.minorVariantCoverageCutoff
                    ):
                        mvCod = (
    ...
    if consensusLen:
        print(
            "Genome coverage: %.2f, mean coverage depth: %.2f (min: %d, "
            "max: %d)\n"
            % (
                (consensusLen - consensusSeq.sequence.count("N")) / consensusLen,
